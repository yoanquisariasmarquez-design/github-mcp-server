...
func TestParseIssueReference(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "missing hash",
			input:       "github/copilot123",
			expectError: true,
		},
		{
			name:        "missing slash",
			input:       "githubcopilot#123",
			expectError: true,
		},
		{
			name:        "empty owner",
		...
		},
		{
			...
			input:       "github/copilot#abc",
			expectError: true,
		},
		{
			name:        "zero issue number",
			input:       "github/copilot#0",
			expectError: true,
		},
		{
			name:        "negative issue number",
			input:       "github/copilot#-1",
			expectError: true,
		},
		{
			name:        "too many hash symbols",
			input:       "github/copilot#123#456",
			expectError: true,
		},
		{
			name:        "too many slashes",
			input:       "github/copilot/extra#123",
			expectError: true,
		},
		{
			name:        "empty string",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if result.Owner != tt.expected.Owner {
				t.Errorf("expected owner %q, got %q", tt.expected.Owner, result.Owner)
			}

			if result.Repo != tt.expected.Repo {
				t.Errorf("expected repo %q, got %q", tt.expected.Repo, result.Repo)
			}
